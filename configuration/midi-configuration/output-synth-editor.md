...
**Reset** the configuration and select the corresponding checkbox. You can uncheck GM as theses 3 standards encompass GM.
...
You may want to prevent JJazzLab from controlling your synth from the mix console, because you directly control it \(e.g. set the instrument and volume on each channel, etc.\).

You need to disable the **Midi parameters** in the **mix console**. You can do it globally via the **Midi menu**: 
...
{% hint style="info" %}
These .ins format extensions are not mandatory to use for JJazzLab to work. But they are required if you have a custom .ins file and want to fully benefit from the JJazzLab features.
{% endhint %}
